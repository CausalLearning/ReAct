...
def test_r2plus1d():
    ...
    config.transformer['backbone']['pretrained2d'] = False
    config.transformer['backbone']['pretrained'] = None
    config.transformer['backbone']['norm_cfg'] = dict(type='BN3d')
...
def test_slowfast():
    ...
    if torch.__version__ == 'parrots':
        ...
        config.transformer.test_cfg['max_testing_views'] = 1
        recognizer = build_recognizer(config.transformer)
        with torch.no_grad():
            img_list = [img[None, :] for img in imgs]
            for one_img in img_list:
...
def test_timesformer():
    ...
    config.transformer['backbone']['pretrained'] = None
    config.transformer['backbone']['img_size'] = 32
