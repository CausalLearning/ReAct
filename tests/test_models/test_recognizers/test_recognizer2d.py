...
def test_tsn():
    ...
    mmcls_backbone = dict(
        ...
        width_per_group=4,
        style='pytorch')
    config.transformer['backbone'] = mmcls_backbone

    recognizer = build_recognizer(config.transformer)
    ...
    config.transformer['backbone']['pretrained'] = None
    recognizer = build_recognizer(config.transformer)
    ...
    config.transformer['backbone'] = tv_backbone
    config.transformer['cls_head']['in_channels'] = 2208
    ...
    config.transformer['backbone'] = timm_backbone
    config.transformer['cls_head']['in_channels'] = 1280
...
def test_tsm():
    ...
    config.transformer.test_cfg = dict(average_clips='prob')
    recognizer = build_recognizer(config.transformer)
...
def test_tpn():
    ...
    # Test forward dummy
    with torch.no_grad():
        _recognizer = build_recognizer(config.transformer)
        img_list = [img[None, :] for img in imgs]
