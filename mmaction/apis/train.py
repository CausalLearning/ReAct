...
from React.utill.StopDataHook import StopDataHook
...
def train_model(model,
    ...
    # modified a bug for mmaction2.
    val_dataloader_setting = dict(dataloader_setting, **cfg.data.get('val_dataloader', {}))
    ...
    if cfg.omnisource:
        ...
        if len(dataset) == 1:
            ...
            data_loaders = [
                build_dataloader(dataset[0], **dataloader_setting),
                build_dataloader(dataset[1], **val_dataloader_setting),
            ]
    ...
    # modified the learning rate
    all_params = model.parameters()
    weight_params = []
    for pname, p in model.named_parameters():
        if pname.endswith('sampling_offsets.weight') or pname.endswith('atten_weight.weight'):
            weight_params += [p]
    ...
    optimizer = torch.optim.AdamW(
        [
            {'params': weight_params, 'lr': 0.1 * learning_rate},
            {'params': other_params}],
        lr=learning_rate
    )
    ...
    # modified optimizer for different learning rate
    ...
    # register hook for contrastive data
    runner.register_hook(StopDataHook(data_loaders[0]))
