...
class NestedTensor(object):
    def __init__(self, tensors, mask: Optional[Tensor], clip_mask=None, snippet_num=None):
    ...
    def to(self, device):
        ...
        if mask is not None:
            ...
            cast_mask = None
        return NestedTensor(cast_tensor, cast_mask, clip_mask)
...
def nested_tensor_from_tensor_list(tensor_list, clip_len=None, snippet_num=None):
    # TODO make this more general
    if isinstance(tensor_list[0], list):
        dim = tensor_list[0][0].shape[1]
        bz = len(tensor_list)
        max_clip_num = np.max([len(feat) for feat in tensor_list])
        batch_shape = [len(tensor_list), max_clip_num, clip_len, dim]
        ...
        for feat, pad_feat, m, clip_m, snip, snip_pad in zip(tensor_list, tensor, mask, clip_mask, snippet_num, snippets_pad):
            clip_m[:len(feat)] = 0
            snip_pad[:len(snip)] = snip
            for i, clip in enumerate(feat):
                pad_feat[i, : feat[i].shape[0], : feat[i].shape[1]].copy_(clip)
                m[i, :feat[i].shape[0]] = False
